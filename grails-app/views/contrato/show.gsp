	...
	<body>
		...
		<div id="show-contrato" class="content scaffold-show" role="main">
			...
			<ol class="property-list contrato">
				...
				<g:if test="${contratoInstance?.clausulas}">
				<li class="fieldcontain">
					<span id="clausulas-label" class="property-label"><g:message code="contrato.clausulas.label" default="Clausulas" /></span>
					
						<g:each in="${contratoInstance.clausulas}" var="c">
						<span class="property-value" aria-labelledby="clausulas-label"><g:link controller="clausula" action="show" id="${c.id}">${c?.encodeAsHTML()}</g:link></span>
						</g:each>
				...
				<li class="fieldcontain">
					<span id="valido-label" class="property-label"><g:message code="contrato.valido.label" default="Valido" /></span>
